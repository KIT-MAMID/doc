...
\begin{description}
	...
	% Replica Set ops
	\oitem{MC}{replica_set_create} The administrator can describe a new MongoDB \gls{replica set} by specifying constraints on 
	how it should be configured by \mamid. \done
	\begin{description}
		\oitem{MC}{replica_set_config_profiles} The administrator can --- on creation of a \gls{replica set} 
		...
		\oitem{MC}{replica_set_member_pv_counts} Volatile and persistent member count of a \gls{replica set} can be independently 
		configured, under constraints described in \ref{F:master_alloc_resp_pv_counts}. \done
	\end{description}
	\oitem{MC}{} The gui visualizes the list of configured \glspl{replica set}. \done
	\oitem{MC}{} The administrator can destroy a \gls{replica set}. \done
	
\end{description}

\subsubsection{MongoDB Configuration \& Monitoring}
\begin{description}
	\oitem{MC}{mongod_deployment1} \mamid asserts that the \glspl{replica set} described by the administrator are configured on the cluster 
	...
	\end{description}
	% Todo old \oitem{MC}{} The master deploys the \gls{replica set} configuration described by the administrator to the cluster.
...
\begin{description}
	...
	% Replica Sets
	\oitem{OC}{deploy_arbiters} \mamid deploys MongoDB arbiters for configured \glspl{replica set} as needed, removing the 
