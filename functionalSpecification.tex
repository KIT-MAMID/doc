...
\begin{description}
	...
	Provides an API for cluster status reporting and administrative activity. %TODO functional refs
 	\oitem{SM}{} Master::Controller\\
	Controls flow of events between different \gls{master} submodules.
	Updates the inventory database (SM30) as state change is recognized by the monitor.
	\oitem{SM}{} Master::Inventory\\
	Database containing
	\begin{itemize}
		\item list of slaves and associated state
		\item list of configured replica sets.
	\end{itemize}
	\oitem{SM}{} Master::ClusterAllocator\\ %TODO funct ref
	It lays out the cluster, i.e. decides the distribution of mongod instances onto cluster hosts.\\
	It respects constraints as described in functional specifications X, Y and Z.\\
	It communicates with the slaves using SM60 to enforce the cluster layout.
	...
	the process manager. It notifies via the MasterSlaveProtocol whether 
