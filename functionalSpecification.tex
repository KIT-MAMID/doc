...
\end{figure}
After clicking the red \textit{Remove slave} button in the \textit{Slave View}, the user is prompted for confirmation (\ref{}). %TODO reference functional requirement
...
\end{figure}
\subsubsection{Removing a Replica Set}
\begin{figure}[H]
	\centering
	\mamidscreenshot{screenshots/replica_set_remove}
	\caption{Removing a replica set}
\end{figure}
After clicking the red \textit{Remove replica set} button in the \textit{Replica Set View}, the user is prompted for confirmation (\ref{}). %TODO reference functional requirement
\begin{itemize}
...
}
{
	\begin{itemize}
		...
		\item The \gls{administrator} deletes $S$ from the \gls{inventory} (in \textit{Slave View}).
...
}
{
	\begin{itemize}
		...
		\item \glslink{administrator}{Administrator} responds to the e-mail notification by opening the \textit{error reports} section in the web interface. %TODO ref
...
}
{
	\begin{itemize}
		...
		\item \glslink{administrator}{Administrator} responds to e-mail notification by opening the \textit{error reports} section in web interface %TODO ref
		\item \hyperref[SM:GUI]{GUI} offers to set $S$ into \glslink{maintenance mode}{maintenance} or \gls{disabled mode}.
...
{
	\begin{itemize}
		\item The \gls{administrator} navigates to the \textit{Slave Management Section} in the \hyperref[SM:GUI]{GUI}. %TODO ui ref
...
{
	\begin{itemize}
		\item The \gls{administrator} navigates to \textit{Replica Set Management Section}. %TODO ref ui
		\item The \gls{administrator} adds a new replica set $R$ using the \textit{Add Replica Set Form} with constraints $p$ and $v$. %TODO ref ui and testsequence
...
[\ref{F:gui_crud_replica_sets}, \ref{F:master_api_crud_repl_set_config}, \ref{F:layout_cluster_config}, \ref{F:master_alloc_resp_pv_counts}, \ref{F:master_alloc_resp_mode}, \ref{F:master_alloc_communicate_config}, \ref{F:slave_control_procs}, \ref{F:slave_enforce_config}, \ref{F:msp_trans_config}]
{
	The \gls{administrator} wants to increase the number of volatile nodes in a replica set. Enough \glspl{slave} with sufficient free \gls{MongoDB} \hyperref[D:slave_mongod_portrange]{ports} are available.
}
{
	\begin{itemize}
		\item The \gls{administrator} navigates to \textit{Replica Set Management Section}. %TODO ref ui
		\item The \gls{administrator} selects the replica set.
		%Sharding config server setting is not modifiable.
		\item The \gls{administrator} increases the number of volatile nodes by 2 and applies the changes.
...
}
{
	\begin{itemize}
		...
		\item The \hyperref[SM:GUI]{GUI} prompts the \gls{administrator} for confirmation. %TODO missing functional requirement
...
{
	The administrator wants to change the hostname of a host
}
{
	\begin{itemize}
		\item The \gls{administrator} navigates to \textit{Slave Management Section}.
		\item The \gls{administrator} selects a slave and clicks the edit button.
		\item The \gls{administrator} sets the slave into maintenance mode.
		\item The \gls{administrator} changes the hostname of the slave in the web interface.
		\item The \gls{administrator} changes the hostname on the host
		\item The \gls{administrator} sets the slave into active mode.
		\item The \hyperref[SM:Master]{Master} reaches the slave under the new hostname.
		\item The \hyperref[SM:Master]{Master} updates the hostname of the slave in the replica sets the slave is part of.
	\end{itemize}
}
...
}
{
	\begin{itemize}
		\item The \gls{administrator} navigates to the \textit{Risk Group Management Section}.
		\item The \gls{administrator} adds the a new risk group using the \textit{Add Risk Group View}.
	\end{itemize}
}
...
}
{
	\begin{itemize}
		\item The \gls{administrator} navigates to the \textit{Risk Group Management Section}.
		\item The \gls{administrator} removes the risk group
	\end{itemize}
}
