...
%TODO state diagram (-> Anton's laptop)
...
\begin{description}
	...
	\end{itemize}
	\oitem{SM}{master_clusterallocator} Master::ClusterAllocator\\ %TODO funct ref
...
\begin{description}
	...
	\oitem{D}{slave_mongod_portrange} mongod-portrange $\in \{{[i, j]} \mid i,j \in \mathbb{N}\}$ (also speportrangecifies how many mongod instances may run on the slave)
...
\begin{description}
	...
	\oitem{F}{gui_set_slave_type} Specify type of slave (\ref{MK:available_slave_types}) on insertion into inventory.
	\oitem{F}{gui_set_root_data_dir} Specify root data directory of slave (\ref{MK:root_data_directory}).
	\oitem{F}{gui_set_phys_interdeps} \acrshort{CRUD} physical interdependencies (\ref{MK:spec_physical_interdep}) between \gls{host}.
	\oitem{F}{gui_set_slave_mode} Set mode of host (\ref{MK:available_slave_modes}).
	...
	\oitem{WF}{gui_auto_discovery_interface} Provide interface to select auto-discovered slaves when adding to inventory (\ref{F:gui_crud_inventory}).
\end{description}

\subsection{Master}
\subsubsection{API Server}
\begin{description}
	\oitem{F}{master_api_crud_slaves} Provide \acrshort{API} to \acrshort{CRUD} slaves in \gls{inventory} (\ref{MK:inventory_definition}).
	\oitem{F}{master_api_crud_phys_interdeps} Provide \acrshort{API} to \acrshort{CRUD} physical interdependencies between \glspl{host}.
	\oitem{F}{master_api_set_slave_mode} Provide \acrshort{API} to set \gls{cluster} host mode (as defined in \ref{MK:available_slave_modes}).
	\oitem{F}{master_api_crud_repl_set_config} Provide \acrshort{API} to \acrshort{CRUD} replica set configurations (\ref{MK:replica_set_create}, \ref{MK:replica_set_config_profiles}, \ref{MK:replica_set_member_counts}).
	...
	\oitem{WF}{master_api_repl_set_status} Provide \acrshort{API} exposing replica set replication status of the configured \glspl{replica set}.
\end{description}
...
\begin{description}
	\oitem{F}{master_inventory_persist_conf_state} Persist and provide the cluster configuration \& state (\gls{inventory}).
\end{description}
\subsubsection{Controller}
\begin{description}
	\oitem{WF}{master_controller_continuous_auto_discovery} Continuously auto-discover \glspl{slave} on the cluster network and add them to the \gls{inventory} (\ref{WK:continuous_autodiscovery}).
...
\begin{description}
	...
	\begin{description}
		\oitem{F}{master_alloc_resp_phys_interdeps} respecting physical interdependencies between \gls{cluster} \glspl{host} (\ref{MK:spec_physical_interdep}).
		\oitem{F}{master_alloc_resp_nums} respecting constraints given by administrator regarding number of persistent \& volatile replica set members per cluster host (\ref{MK:replica_set_member_counts} and \ref{MK:available_slave_types}).
		\oitem{F}{master_alloc_resp_mode} respecting the mode of hosts (\ref{MK:available_slave_modes}).
		\oitem{WF}{master_alloc_arbiters} adding \gls{MongoDB} \glspl{arbiter} where necessary (\ref{WK:deploy_arbiters}).
	\end{description}
	\oitem{F}{master_alloc_communicate_config} Communicate the \gls{MongoDB} instance configuration to the \glspl{slave} (\ref{MK:deploy_replica_set_config}).
\end{description}

\subsection{MasterSlaveProtocol}
\begin{description}
	\oitem{F}{msp_trans_config} Transport \gls{replica set} configuration description of \gls{cluster} \glspl{host}.
	\oitem{F}{msp_trans_rechability} Transport reachability check messages.
...
\begin{description}
	% Controller
	\oitem{F}{slave_enfoce_config} Enforce configuration description received from \gls{master} through \ref{SM:masterslaveproto} (also: \ref{MK:deploy_replica_set_config}). 
	% Process manager
	\oitem{F}{slave_control_procs} Spawn / Control / Kill \gls{MongoDB} processes.
	% MongoDB Client
	\oitem{F}{slave_communicate_with_mongodb} Communicate with \gls{MongoDB} processes using \ref{SM:mongodbclient}.
	% Controller again
	\oitem{F}{slave_report_config} Report current configuration through \ref{SM:masterslaveproto}.
	\oitem{WF}{slave_report_machine_metrics} Report machine metrics through \ref{SM:masterslaveproto}.
	\oitem{WF}{slave_report_repl_status} Report \gls{replica set} replication status through \ref{SM:masterslaveproto}.
\end{description}
...
\begin{description}
	...
	\oitem{F}{notifmgr_read_conf_file} Read configured list of contacts from a text-based configuration file.
	% Mail Notifier
	\oitem{F}{notifmgr_send_emails} Send error reports via \glslink{email}{e-mail messages} to a contact's email address.
\end{description}
...
\begin{description}
	...
	\oitem{WF}{cli_set_slave_type} Specify type of slave (\ref{MK:available_slave_types}) on insertion into inventory.
	\oitem{WF}{cli_set_root_data_dir} Specify root data directory of slave (\ref{MK:root_data_directory}).
	\oitem{WF}{cli_set_phys_interdeps} \acrshort{CRUD} physical interdependencies (\ref{MK:spec_physical_interdep}) between \gls{host}.
	\oitem{WF}{cli_set_slave_mode} Set mode of host (\ref{MK:available_slave_modes}).
...
\begin{description}
	...
	\end{itemize}
	\oitem{NF}{} Team Communication: Slack, GitHub, \glslink{email}{E-Mail}
...
}
{
	\begin{itemize}
		\item The \gls{administrator} navigates to the 'slave management section' in the \hyperref[SM:GUI]{GUI}.
		\item The \gls{administrator} selects the slave $S$ and sets it to \gls{disabled mode}. %TODO ref testseq and ui
		\item The \hyperref[SM:Master]{master} determines a new cluster layout to repair $S$'s former \glspl{replica set}.
		\item The \hyperref[SM:Master]{master} communicates the desired configuration to the appropriate \glspl{slave}.
		\item The \hyperref[SM:GUI]{GUI} now allows the \gls{administrator} to delete the slave $S$ from the \gls{inventory}.
		\item The \gls{administrator} deletes the slave $S$ from the \gls{inventory}. %TODO ref testseq and ui
		\item The \gls{administrator} can now physically remove $M$ from the cluster.
	\end{itemize}
}
...
\testsequence
[\ref{F:master_monitor_config}, \ref{F:master_api_error_reports}, \ref{F:gui_display_errors}, \ref{F:gui_set_slave_mode}, \ref{F:msp_trans_rechability}]
{
	A slave becomes unreachable because of broken network cable.
}
{
	\begin{itemize}
		...
		\item \hyperref[SM:GUI]{GUI} offers to set slave into \glslink{maintenance mode}{maintenance} or \gls{disabled mode}.
...
\testsequence
[\ref{F:master_monitor_config}, \ref{F:master_api_error_reports}, \ref{F:gui_display_errors}, \ref{F:gui_set_slave_mode}, \ref{F:master_api_set_slave_mode}, \ref{F:master_inventory_persist_conf_state}, \ref{F:layout_cluster_config}, \ref{F:master_alloc_resp_nums}, \ref{F:master_alloc_resp_mode}, \ref{F:master_alloc_communicate_config}, \ref{F:slave_control_procs}, \ref{F:slave_enfoce_config}, \ref{F:slave_report_config}, \ref{F:msp_trans_config}, \ref{F:msp_trans_rechability}, \ref{F:master_monitor_config}]
{
	A slave $S$ becomes permanently unreachable because there was a short-circuit on its mainboard which needs to be replaced. The admin has no spare mainboards left and has to wait for replacement hardware to arrive. However, there is at least one \glslink{active mode}{active} slave allowed to take over the role of the failed one.
}
{
	\begin{itemize}
...
\testsequence
[\ref{F:gui_crud_inventory}, \ref{F:master_api_crud_slaves}, \ref{F:gui_set_slave_mode}, \ref{F:master_api_set_slave_mode}, \ref{F:master_inventory_persist_conf_state}, \ref{F:master_alloc_communicate_config}, \ref{F:slave_control_procs}, \ref{F:slave_enfoce_config}, \ref{F:slave_report_config}, \ref{F:msp_trans_config}]
{
	A slave $S$ needs to be rebooted to install security updates to the installed software.
}
{
	\begin{itemize}
		...
		\item The \gls{administrator} installs the security updates and reboots the machine (killing the \gls{slave} instance and all \gls{MongoDB} instances).
		...
		\item $S$ starts the \gls{MongoDB} processes and configures replica set membership if required by the communicated configuration.
	\end{itemize}
}
...
\testsequence
[\ref{F:gui_crud_replica_sets}, \ref{F:master_api_crud_repl_set_config}, \ref{F:layout_cluster_config}, \ref{F:master_alloc_resp_nums}, \ref{F:master_alloc_resp_mode}, \ref{F:master_alloc_communicate_config}, \ref{F:slave_control_procs}, \ref{F:slave_enfoce_config}, \ref{F:msp_trans_config}]
{
	The \gls{administrator} wants to add a replica set with $p \in \mathbb{N}$ persistent members and $v \in \mathbb{N}$ volatile members. $(p+v) >= 3 \land (p+v) is odd$. Enough \glspl{slave} with sufficient free \gls{MongoDB} \hyperref[D:slave_mongod_portrange]{ports} are available.
}
{
	\begin{itemize}
		\item Navigate to 'replica set management section' %TODO ref ui
		\item Add a new replica set $R$ using the 'add replica set form' with constraints $p$ and $v$. %TODO ref ui and testsequence
...
[\ref{F:gui_crud_replica_sets}, \ref{F:master_api_crud_repl_set_config}, \ref{F:layout_cluster_config}, \ref{F:master_alloc_resp_nums}, \ref{F:master_alloc_resp_mode}, \ref{F:master_alloc_communicate_config}, \ref{F:slave_control_procs}, \ref{F:slave_enfoce_config}, \ref{F:msp_trans_config}]
{
	The \gls{administrator} wants to destroy a \gls{replica set}.
}
{
	\begin{itemize}
		\item Navigate to 'replica set management section' %TODO ref ui
		\item The \gls{administrator} selects the replica set $R$ and clicks the remove button % TODO ref testseq and ui
		...
		\item The \hyperref[SM:Master]{master} persists the change and communicates to the \glspl{slave} that host members of $R$ to kill the respective \gls{MongoDB} processes.
