...
\begin{description}
	...
	Provides an API for \gls{cluster} status reporting and administrative activity. %TODO functional refs
	...
	API client for \ref{SM:masterapiserver}.
...
\subsection{GUI}
The GUI acts as a frontend to the functionality provided by \ref{SM:masterapiserver}. Hence, all functionality described in this subsection is realized through API calls to the \gls{master}.
\begin{description}
	\oitem{F}{} \acrshort{CRUD} list of \glspl{slave} (\gls{inventory}).
...
\begin{description}
	% API Server
	\oitem{F}{} Provide API to \acrshort{CRUD} list of slaves in \gls{inventory} (\ref{MK:inventory_definition}).
	\oitem{F}{} Provide API to \acrshort{CRUD} physical interdependencies between \glspl{host}.
	\oitem{F}{} Provide API to set \gls{cluster} host mode (as defined in \ref{MK:available_slave_modes}).
	\oitem{F}{} Provide API to \acrshort{CRUD} replica set configurations (\ref{MK:replica_set_create}, \ref{MK:replica_set_config_profiles}, \ref{MK:replica_set_member_counts}).
	\oitem{F}{master_api_error_reports} Provide API to retrieve error reports (\ref{MK:detect_slave_unexpected_behavior}).
	...
	% Controller
	% Cluster Allocator
		\oitem{F}{} Lay out the cluster configuration, i.e. decide on a \gls{replica set} configuration
	\begin{description}
...
\begin{description}
	% API Client
	\oitem{F}{} Consume error reporting API of \gls{master} \ref{F:master_api_error_reports}.
	% Controller
