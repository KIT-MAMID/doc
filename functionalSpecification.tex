...
\oitem{MK}{available_slave_types} The \gls{administrator} can set a \gls{slave} as either \glslink{persistent storage}{persistent} or \glslink{volatile storage}{volatile}.
...
\begin{description}
	...
	It respects constraints as described by \ref{F:master_alloc_resp_phys_interdeps}, \ref{F:master_alloc_resp_nums}, \ref{F:master_alloc_resp_mode} and \ref{WF:master_alloc_arbiters}.\\
...
\begin{itemize}
	...
	\mamid will monitor \glspl{slave} in active mode. If the slave behaves unexpectedly, i.e. is not reachable, \mamid will notify the \gls{administrator} and set its state to unknown.
	\item unknown\\
	\mamid will monitor \glspl{slave} in unknown state and set them into active mode if they recover. The \gls{administrator} can set the slave state to maintenance to stop monitoring of the slave or to disabled mode to replace the slave.
	\item \glslink{maintenance mode}{maintenance}\\
	\mamid will not monitor \glspl{slave} in maintenance mode, but will not alter any \gls{replica set}.
	\item \glslink{disabled mode}{disabled}\\
	\mamid will dissociate \glspl{slave} in disabled mode with any \glspl{replica set} they may be member of.
\end{itemize}
...
\begin{description}
	...
	\oitem{F}{master_api_crud_repl_set_config} Provide \acrshort{API} to \acrshort{CRUD} \gls{replica set} configurations (\ref{MK:replica_set_create}, \ref{MK:replica_set_config_profiles}, \ref{MK:replica_set_member_counts}).
...
\begin{description}
	...
	\begin{description}
		...
		\oitem{F}{master_alloc_resp_nums} respecting constraints given by administrator regarding number of \glslink{persistent storage}{persistent} \& \glslink{volatile storage}{volatile} \gls{replica set} members per \gls{cluster} host (\ref{MK:replica_set_member_counts} and \ref{MK:available_slave_types}).
...
\begin{description}
	% Controller
	\oitem{F}{slave_enforce_config} Enforce configuration description received from \gls{master} through \ref{SM:masterslaveproto} (also: \ref{MK:deploy_replica_set_config}). 
	% Process manager
