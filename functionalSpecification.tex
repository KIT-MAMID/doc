...
% replica set ops
\oitem{MK}{replica_set_create} The \gls{administrator} can describe a new \gls{replica set} by specifying constraints on how it should be configured by \mamid.
\begin{description}
...
\oitem{WK}{monitor_icmp} \mamid can recognize when the \gls{slave} software does not respond but the corresponding \gls{host} is still connected to the network.
...
\oitem{WK}{} The \gls{administrator} can specify a \gls{MongoDB} configuration file template for \glspl{slave}.
