...
\begin{itemize}
	...
	\mamid will monitor slaves in active mode. If the slave behaves unexpectedly, i.e. is not reachable, \mamid will notify the \gls{administrator} and set its state to unknown.
	\item unknown\\
	\mamid will monitor slaves in unknown state and set them into active mode if they recover. The administrator can set the slave state to maintenance to stop monitoring of the slave or disabled mode to replace the slave.
	\item \glslink{maintenance mode}{maintenance}\\
	\mamid will not monitor slaves in maintenance mode.
	\item \glslink{disabled mode}{disabled}\\
	\mamid will dissociate slaves in disabled mode with any replica sets they may be member of
\end{itemize}
