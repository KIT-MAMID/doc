\section{Slave}
The slave is installed on the individual host nodes and to be started up by the main process manager of the operating system after boot.

\subsection{Application / main()}{
  The Application is the main function. It does early initialization:
  \begin{itemize}
    \item Parses command line flags using an external CLIFlagsParser
    \item Initializes the Controller with a concrete instance of MongodConfigurator
    \item Initializes the MSPListener and passes it the Controller and calls its run method
  \end{itemize}
}

\struct{MSPListener}{
  General class managing communication to the exterior
}{
  ...
  \method{run}{
    ...
    \begin{itemize}
      \item Broadcasting availability of the slave
