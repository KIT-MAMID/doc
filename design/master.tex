...
All datastructures in \refgo{model} are designed to be fully serializable \& usable with a database abstraction layer.
...
\struct{Slave}{
  ...
  \property{Port}[uint]{Port on which the slave listens for connections from the master.}
  \property{MongodPortRangeBegin}[uint]{Beginning of the range of ports \refstruct{Mongod} spawned by the slave may listen on (inclusive).}
  \property{MongodPortRangeEnd}[uint]{End of the range of ports \refstruct{Mongod} spawned by the slave may listen on (exclusive).
      Implicitly specifies --- in combination with \refproperty{MongodPortRangeBegin} ---
      how many concurrently running \refstruct{Mongod}s can be spawned by a slave.}
  ...
  \property{RiskGroup}[\refstruct{RiskGroup}]{The risk group the slave is a member of.}
  \property{Mongods}[[]\refstruct{Mongod}]{The \refstruct{Mongod}s that are running or should be running on this slave.
      Note that there may exist \refstruct{Mongods} that do not have a \refstruct{ParentSlave}.}
...
{}
{
	...
	\goenumitem{Disabled}{Slaves in disabled mode are not monitored.
...
\struct{RiskGroup}{
  A \refstruct{RiskGroup} models a set of slaves sharing a common risk of failure.
  
  A \refstruct{Slave} always belongs to exactly one \refstruct{RiskGroups}.
  Hence, \codeinline{RiskGroup}s are mutually disjoint.
}{
  \property{Id}[uint]{Unique id.}
  \property{Name}[string]{Unique name of the risk group.}
  \property{Slaves}[[]\refstruct{Slave}]{The \refstruct{Slave}s that are members of a the given instance of \refstruct{RiskGroup}.}
}
...
\subsection{Type Alias \codeinline{PortNumber $\equiv$ uint16}}\label{\gocurpackage.PortNumber}
...
\struct{DesiredReplicaSetConstraintStatus}
{
  ...
  \begin{itemize}
    \item the persistent and volatile member counts of the \refstruct{ReplicaSet}
    \item the maximum number of \refstruct{Mongod}s on the \refstruct{Slave}s
    \item the \refstruct{RiskGroup}s
  \end{itemize}
}{
  \property{Unsatisfied}[bool]{Whether the constraint cannot be satisfied.}
  \property{ReplicaSet}[\refstruct{ReplicaSet}]{The Replica Set whose constraints can/cannot be fulfilled}
  \property{ActualVolatileCount}[uint]{The count of volatile members the Replica Set now has because the constraints are not fulfilled. If they are fullfilled this is the same as \refstruct{ReplicaSet.VolatileMemberCount}.}
  \property{ActualPersistentCount}[uint]{The count of persistent members the Replica Set now has because the constraints are not fulfilled. If they are fullfilled this is the same as \refstruct{ReplicaSet.PersistentMemberCount}.}
}

\struct{MongodMatchStatus}
{
  Represents whether the observed and desired state of a \refstruct{Mongod} do not match.
}{
  \property{Mismatch}[bool]{Whether the stated do not match.}
  \property{Mongod}[\refstruct{Mongod}]{The mongod whose states do/do not match}
}

\struct{ConnectionStatus}
{
  Represents whether a \refstruct{Slave} cannot be reached from the master.
}{
  \property{Mismatch}[bool]{Whether the slave cannot be reached.}
  \property{Slave}[\refstruct{Mongod}]{The slave that can/cannot be reached}
...
{
  Represents whether the observed member counts of a \refstruct{ReplicaSet} do not match \refstruct{ReplicaSet.VolatileMemberCount} and \refstruct{ReplicaSet.PersistentMemberCount}
}{
  \property{Unsatisfied}[bool]{Whether the counts do not match.}
  \property{ReplicaSet}[\refstruct{ReplicaSet}]{The Replica Set whose counts do not match.}
