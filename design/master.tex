...
\struct{Mongod}{
  ...
  \property{ObservationError}[error]{Latest error that occured when attempting to update the \refproperty{Mongod.ObservedState}.\\
                                     \codeinline{nil} if no error occured on last attempt.}
...
\goenum{MongodExecutionState}{
  ...
  \goenumitem{Recovering}{Mongod running but not available for reads because data needs to be synced from other Replica Set members. Common when adding new members to an existing Replica Set.}
