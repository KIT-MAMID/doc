...
\NewDocumentCommand{\structure}{m m g g}{
  \subsection{#1}
  #2
  \IfValueT{#4}{
...
\NewDocumentCommand{\struct}{m m g g}{\structure{Struct #1}{#2}{#4}{#3}}
\newcommand{\interface}[3]{\structure{Interface #1}{#2}{#3}}

\NewDocumentCommand{\property}{m o m+}{
  \item{\textbf{#1\IfValueT{#2}{: #2}} #3}
}
\NewDocumentCommand{\method}{m o d() m+}{
  \item[]{\textbf{#1\IfValueT{#2}{: #2}}}
  \IfValueT{#3}{
    \begin{itemize}
      #3
    \end{itemize}
...
\NewDocumentCommand{\param}{m d<> m}{
  \item{\textbf{\IfValueT{#2}{#2: }#1} #3}
}
